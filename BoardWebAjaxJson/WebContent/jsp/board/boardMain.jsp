...
%>
    ...
    <body>
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
            <div class="container">
                <a class="navbar-brand" href="#"><img src="<%= contextPath + userDto.getUserProfileImageUrl()%>" 
    ...
    <script>
    	...
    	window.onload = function(){
            ...
            //update	
           	document.querySelector("#btnBoardUpdateForm").onclick=function () {
            	...
            	//detail  modal 닫고 update modal을 띄운다
            	let boardId=document.querySelector("#boardDetailModal").getAttribute("data-boardId");	 //detail Modal에 있는 boardID 가져옴 -> 수정하기 위해
        ...
        async function boardDelete(){
            ...
            try{
                let response = await fetch( url + urlParams); // default GET
                let data = await response.json(); // json => javascript object <= JSON.parse()
                if( data.result == "success"){ // login.jsp => boardMain.jsp 로 페이지 이동 ( 새로운 페이지(html....) 요청)
